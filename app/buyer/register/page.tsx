...
export default function BuyerRegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await signUp(formData.email, formData.password, formData.name, formData.phone || undefined);
  ...
  return (
    ...
    >
      <StoreHeader storeName={storeName} theme={theme} logo={storeData?.website?.logo} />
            ...
            >
              <div className="text-center mb-8">
                ...
                <p style={{ color: theme.textColor, opacity: 0.7 }}>
                  Join the community at {storeName || 'the store'}
                </p>
              </div>
              ...
              <div className="mt-6 text-center">
                <p className="text-sm" style={{ color: theme.textColor, opacity: 0.7 }}>
                  ...
                  <Link
                    href={`/buyer/login${returnUrl !== '/' ? `?returnUrl=${encodeURIComponent(returnUrl)}` : ''}`}
