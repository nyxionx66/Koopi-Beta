...
import { X, Download, Sparkles, Instagram, Facebook, Twitter, Share2, Palette, Check, Image as ImageIcon, ArrowRight } from 'lucide-react';
...
export default function SocialMediaKit({ storeName, storeUrl, onClose }: SocialMediaKitProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      const svgDataUrl = await domtoimage.toSvg(cardRef.current, {
        width: 1080,
        height: 1080,
      });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-lg p-4 sm:p-6 lg:p-8" onClick={onClose}>
      <motion.div
        ref={modalRef}
        ...
        className="relative w-full h-full max-w-7xl bg-white/70 backdrop-blur-2xl border border-gray-200/50 rounded-3xl shadow-2xl flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex-shrink-0 flex items-center justify-between p-4 sm:p-6 border-b border-gray-200/80">
        ...
        <div className="flex-grow grid grid-cols-1 lg:grid-cols-3 gap-px bg-gray-200/80 overflow-hidden">
          {/* Left Side - Preview */}
          <div className="col-span-2 bg-gray-50/80 flex flex-col items-center justify-center p-4 sm:p-8 lg:p-12">
            ...
            >
              <div 
                ref={cardRef}
                className="w-full h-full rounded-2xl overflow-hidden shadow-2xl border-2 border-white"
              >
